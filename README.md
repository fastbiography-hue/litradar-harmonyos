...
LitRadar 是首个基于 HarmonyOS PC 平台的生物医学文献追踪工具，支持**多数据源并行检索**、**智能相关性评分**、**本地离线缓存**、**定时自动抓取**与**鸿蒙通知中心推送**。
...
- 🔬 **多源并行检索**：原生直连 PubMed · Europe PMC 两大公开 API，多源并发拉取，自动去重合并
- 🧠 **智能评分引擎**：基于关键词命中、期刊影响力、被引次数、发表年份的 0–10 分自动打分算法，与 Python 版偏差 ≤ 0.1 分
- 💾 **本地离线优先**：所有文献持久化到鸿蒙本地 RDB，无网络时可浏览历史数据，离线不掉线
- ⏰ **定时自动抓取**：基于 WorkScheduler 的后台每日任务，设定时间自动检索新文献，无需手动操作
- 🔔 **通知中心推送**：后台抓取完成后推送系统通知——"今日新增 X 篇，其中高分文献 Y 篇"
- 🎨 **三栏交互布局**：左栏检索策略 · 中栏文献列表 · 右栏详情摘要，信息密度高，操作路径短
